...
public static class Result
{
        ...
        where T1: struct, IError<T1>
    {
        ...
        (var t_0, t1) = ret.Result.Either.Value;
        if(t_0 is not null)
        {
        ...
        where T : struct, IResult<T>
    {
        ...
        {
            return new Try<T>() { Result = T.FromException(exception) };
        }
    }
...
{
    public static abstract TSelf FromException(Exception ex);
}
...
//AutoGenerated
readonly partial struct Result<T0, TError> : IResult<Result<T0, TError>> 
    where T0 : notnull
    where TError : struct, IError<TError>
{
    public required Either<T0, TError> Either { get; init; }

    public static Result<T0, TError> FromException(Exception ex)
    {
        TError.Matches(ex, out var error);
        return error;
    }
...
readonly partial struct Result<T0, TError0, TError1> : IResult<Result<T0, TError0, TError1>> 
    where T0 : notnull
    ...
    where TError1 : struct, IError<TError1>
{
    ...
    public static Result<T0, TError0, TError1> FromException(Exception ex)
    {
        if(TError0.Matches(ex, out var error0))
        {
            return error0;
        }

        TError1.Matches(ex, out var error1);
        return error1;
    }

    public static implicit operator Result<T0, TError0, TError1>(T0 value) => new () { Value = value };
    public static implicit operator Result<T0, TError0, TError1>(TError0 value) => new () { Value = value };
    public static implicit operator Result<T0, TError0, TError1>(TError1 value) => new () { Value = value };
}
